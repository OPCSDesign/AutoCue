# AutoCue ↔ Nextcloud setup

AutoCue can keep its whole library in your Nextcloud so every device reads the
same content, while still caching everything locally for offline use.
...
- Create, rename, move, reorder and delete in any Nextcloud client; AutoCue
  picks changes up on its next sync. Text edits made inside AutoCue are saved
...
```nginx
# AutoCue browser access to WebDAV
...
1. In Nextcloud: Settings → Security → Devices & sessions → create an **app
   password** (name it e.g. “AutoCue phone”). One per device; revocable
   individually.
2. In AutoCue: ☁ Nextcloud → enter server address, username, app password →
   Connect.
